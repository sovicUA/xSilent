...
- [x] **Середні знахідки код-рев'ю (2026-09-02, гілка `fix/review-medium-findings`).**
  ...
  - `main()`: прибрано окремий `await notificationService.init()` до `runApp`. Bootstrap сповіщень — на першому (негайному) спрацюванні `container.listen(effectiveLocaleProvider)` → `reconcile()`. **`reconcile()` тепер робить `_notifications.init()` першим рядком** (до сіду через `create()`, який усередині викликає `sync()`) — інакше на свіжій інсталяції перший `sync()` летів на неініціалізованому плагіні й `reconcile()` падав до `init()`/`syncAll()` (0 alarm, без каналу `spoken_r1`; зловлено смоук-тестом на Motorola). На порожній БД після `create()` — `return` (без зайвого повторного `syncAll`).
...
- [ ] **3 типи сповіщень (вбудоване / нагадування / будильник) + вибір голосу TTS.**
  За фідбеком внутрішнього тесту 1.1.0. Повний аналіз, модель даних (schema v7),
  стратегії реалізації будильника (A: ланцюг сповіщень / B: нативний
  foreground-сервіс), поетапний план і оцінки — у
  [PLAN-notification-types.md](PLAN-notification-types.md). Область — лише Android.
  - [x] **Вибір голосу TTS** — `41bfae4` (розділ «Голос озвучення» в
    налаштуваннях, `TtsVoice`, глобальний `tts_voice`, голос у хеші каналу).
  - [ ] Спайк: чи зупиняє `cancel()` звук каналу; `fullScreenIntent` на локскріні.
  - [ ] Модель 3 типів (schema v7) + UI-каркас.
  - [ ] Поведінка будильника (Стратегія A).
